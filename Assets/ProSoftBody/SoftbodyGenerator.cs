...
public class SoftbodyGenerator : MonoBehaviour
{
    ...
    [HideInInspector] public List<GameObject> phyisicedVertexes;
    [HideInInspector] public List<Rigidbody> verticesRbs;
    ...
    private void Awake()
    {
        ...
        LayerMask verticesMask = LayerMask.NameToLayer("SoftbodyVertices");
        ...
        foreach (var vertecs in _optimizedVertex)
        {
            ...
            _tempObj.AddComponent<JointFix>();
            
            
            phyisicedVertexes.Add(_tempObj);
            verticesRbs.Add(_tempRigidBody);
        }
    ...
    {
        foreach(var rb in verticesRbs)
        {
            rb.AddForce(force);
        }
        //foreach (var gObject in phyisicedVertexes)
           // gObject.GetComponent<Rigidbody>().AddForce(force);
    }

    public Transform GetTrueTransform()
    {
        if(centerOfMasObj) return centerOfMasObj.transform;
        return null;
    }
